...
def process():
    logger.info("TestGetIndex.py process~~~")#记录日志
    ...
    requrl = ENV.getBaseUrl() + "/api/getIndex";#拼接首页的URL
    ...
    assert 7 == len(response_json['data']['classifyList'])
    assert 7 <= len(response_json['data']['newClassifyList'])
    assert 6 <= len(response_json['data']['news'])
    assert 5 <= len(response_json['data']['recommendsList'])
